...
export function BookingForm({ vendorName, onSubmit, isLoading = false }: BookingFormProps) {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    setFormData((prev) => ({
      ...prev,
      [name]: name === "guestCount" || name === "proposedPrice" ? (value ? Number.parseInt(value) : undefined) : value,
    }))
  }
  ...
  return (
    <Card className="p-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <label className="block text-sm font-medium text-foreground mb-2">Precio Propuesto (opcional)</label>
          <Input
            type="number"
            name="proposedPrice"
            ...
            min="0"
          />
          <p className="text-xs text-muted-foreground mt-1">Si tienes un presupuesto en mente, compártelo aquí</p>
        </div>
